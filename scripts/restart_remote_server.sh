...
echo "Stopping existing zeta-zero-server..."
ssh $SERVER "pkill -f zeta-zero-server || true"

echo "Waiting for process to terminate..."
sleep 2

echo "Starting new zeta-zero-server..."
# Start in background, redirect output to log file
ssh $SERVER "cd $REMOTE_BUILD_DIR && nohup ./zeta-zero-server > $LOG_FILE 2>&1 &"
