...
# Configuration
SERVER = os.getenv("ZETA_SERVER", "http://192.168.0.213:9000")
...
def generate(prompt, max_tokens=600, timeout=420):
    try:
        # Use OpenAI-compatible chat completions API
        messages = [{"role": "user", "content": prompt}]
        payload = {
            "model": "security-llama3.2-3b.Q4_K_M.gguf",
            "messages": messages,
        ...
        if r.status_code == 200:
            ...
            return {"response": "No response content", "error": True}
...
def phase8_gauntlet():
    ...
    expected = [
        ["391"], ["53", "59", "61"], ["canberra"], ["fruit"], ["ff0000"], ["8"], ["hypertext", "transfer", "protocol"],
        ["299792458"], ["adenine", "thymine", "cytosine", "guanine"], ["h2o"], ["vinci"], ["jupiter"], ["100"], ["8"], ["12"],
        ["functional", "object-oriented"], ["6"], ["1945"], ["x^3/3"], ["81"]
    ]
    ...
    for i, (query, exp) in enumerate(zip(queries, expected)):
        ...
        passed = all(keyword.lower() in response for keyword in exp)
        ...
        test_results["tests"].append(
            {
                ...
                "passed": passed,
                "reasons": [] if passed else [f"Missing: {', '.join(exp)}"],
            }
        )
        log(f"    {i+1}. {status} {query[:40]}... -> {response[:50]}")
