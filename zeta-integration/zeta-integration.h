...
// Generate attention mask with temporal decay baked in
int zeta_kernel_generate_mask(
    zeta_context_t* ctx,
    float* mask,
    int seq_len,
    int kv_len,
...
// Inject memory attention into output (superposition)
int zeta_kernel_memory_injection(
    zeta_context_t* ctx,
    float* output,
...
// Apply sparse softmax (softmax + zeroing small values)
int zeta_kernel_sparse_softmax(
    zeta_context_t* ctx,
    float* scores,
    int seq_len,
    int kv_len,
