...
static bool zeta_embed_init(const char* model_path) {
    ...
    cparams.flash_attn = false;    // Embedding models don't need flash attn
    ...
    g_embed_ctx->embed_dim = llama_n_embd(g_embed_ctx->model);
...
static int zeta_embed_text(const char* text, float* output, int max_dim) {
    ...
    // Clear KV cache
    llama_kv_cache_clear(g_embed_ctx->ctx);
    ...
    for (int i = 0; i < n_tokens; i++) {
        ...
        batch.n_seq_id[i] = 1;
        batch.seq_id[i][0] = 0;
        batch.logits[i] = (i == n_tokens - 1) ? 1 : 0;  // Only need last token's embedding
    }
