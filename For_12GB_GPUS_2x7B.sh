...
MODELS_DIR="${MODELS_DIR:-$HOME/models/zetazero-2x7b}"
ZETA_STORAGE_DIR="${ZETA_STORAGE_DIR:-$HOME/.zetazero/HoloGit/blocks}"
UI_DIR="$ROOT/ui/for-8gb-gpus-chat"

MODEL_MAIN_NAME="${MODEL_MAIN_NAME:-qwen2.5-7b-instruct-q4_k_m.gguf}"
...
cat <<EOF
== For ~12GB GPUs (2x7B) ==
Downloads models via wget, starts llama-zeta-server on :${PORT_ZETA},
creates local HoloGit storage, and opens the chat-only UI.

Required URLs:
  export MODEL_MAIN_URL='...7b-instruct...gguf'
  export MODEL_3B_URL='...7b-coder...gguf'

Optional:
  export EMBED_URL='...gte...gguf'
  export MODEL_3B_CODER_URL='...7b-coder...gguf'  # used for code mode switching
...
LOG_FILE="$ROOT/zeta_server_2x7b.log"
...
set +e
nohup "$ZETA_BIN" "${args[@]}" >"$LOG_FILE" 2>&1 &
ZETA_PID=$!
set -e
...
nohup python3 -m http.server "$PORT_UI" --directory "$UI_DIR" >"$ROOT/zeta_ui_2x7b.log" 2>&1 &
UI_PID=$!
