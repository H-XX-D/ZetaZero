...
typedef struct {
    ...
    float    logit_softcap;
    // Z.E.T.A. parameters
    float    zeta_temporal_lambda;    // Temporal decay rate (0 = disabled)
    float    zeta_tunneling_threshold; // Sparse gating threshold (-1e10 = disabled)
    // Z.E.T.A. memory block injection (superposition)
    int32_t  zeta_mem_tokens;         // Total tokens across all memory blocks (0 = disabled)
    int32_t  zeta_mem_head_dim;       // Head dimension of memory blocks
    float    zeta_mem_alpha;          // Injection weight (entanglement score)
