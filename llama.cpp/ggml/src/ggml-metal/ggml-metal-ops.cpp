...
// Z.E.T.A. parameter helpers
static float zeta_get_temporal_lambda() {
    const char* env = std::getenv("ZETA_TEMPORAL_LAMBDA");
    return env ? std::strtof(env, nullptr) : 0.0f;
}

static float zeta_get_tunneling_threshold() {
    const char* env = std::getenv("ZETA_TUNNELING_THRESHOLD");
    return env ? std::strtof(env, nullptr) : -1e10f;
}

// Z.E.T.A. memory block injection state (global for now, will be context-specific)
static int32_t g_zeta_mem_tokens = 0;
static int32_t g_zeta_mem_head_dim = 0;
static float   g_zeta_mem_alpha = 0.0f;

static int32_t zeta_get_mem_tokens() { return g_zeta_mem_tokens; }
static int32_t zeta_get_mem_head_dim() { return g_zeta_mem_head_dim; }
static float   zeta_get_mem_alpha() { return g_zeta_mem_alpha; }

// API to set memory block parameters (called before inference)
extern "C" void zeta_metal_set_memory_params(int32_t tokens, int32_t head_dim, float alpha) {
    g_zeta_mem_tokens = tokens;
    g_zeta_mem_head_dim = head_dim;
    g_zeta_mem_alpha = alpha;
}
...
#define FATTN_SMEM(nsg) (GGML_PAD((nqptg*(ne00 + 2*GGML_PAD(ne20, 64) + 2*(2*ncpsg)) + is_q*(16*32*(nsg)))*(sizeof(float)/2), 16))
        ...
        ggml_metal_kargs_flash_attn_ext args = {
            ...
            // Z.E.T.A. parameters (from env vars or disabled by default)
            /*.zeta_temporal_lambda    =*/ zeta_get_temporal_lambda(),
            /*.zeta_tunneling_threshold=*/ zeta_get_tunneling_threshold(),
            // Z.E.T.A. memory injection (disabled by default, set via zeta_set_memory_blocks)
            /*.zeta_mem_tokens         =*/ zeta_get_mem_tokens(),
            /*.zeta_mem_head_dim       =*/ zeta_get_mem_head_dim(),
            /*.zeta_mem_alpha          =*/ zeta_get_mem_alpha(),
        };
