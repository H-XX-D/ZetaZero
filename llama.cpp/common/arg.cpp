...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--zeta-lambda"}, "N",
        string_format("Z.E.T.A.: temporal decay rate for attention (default: %.2f, 0 = disabled)", (double)params.zeta_temporal_lambda),
        [](common_params & params, const std::string & value) {
            params.zeta_temporal_lambda = std::stof(value);
        }
    ).set_env("LLAMA_ARG_ZETA_LAMBDA"));
    add_opt(common_arg(
        {"--zeta-tau"}, "N",
        string_format("Z.E.T.A.: sparse gating threshold (default: %.2f, -1e10 = disabled)", (double)params.zeta_tunneling_threshold),
        [](common_params & params, const std::string & value) {
            params.zeta_tunneling_threshold = std::stof(value);
        }
    ).set_env("LLAMA_ARG_ZETA_TAU"));
    add_opt(common_arg(
        {"-gan", "--grp-attn-n"}, "N",
